// page_deko.jsx — Analyse sailor82
...
function DekoPage({ onBack }) {
  ...
  const load = React.useCallback(async () => {
    ...
    const [pos, rap, live] = await Promise.all([
      sbFetch('positions_tracker', 'trader=eq.sailor82&order=detected_at.desc&limit=200'),
      sbFetch('tracker_rapports',  'trader=eq.sailor82&order=created_at.desc&limit=5'),
  ...
  return (
    <div>
      ...
      <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:'var(--gap)'}}>
        ...
        <div style={{width:46,height:46,borderRadius:14,background:'color-mix(in oklab,#6366f1 15%,var(--bg-elev))',border:'1.5px solid color-mix(in oklab,#6366f1 40%,transparent)',display:'grid',placeItems:'center',fontSize:22}}>🔍</div>
        <div style={{flex:1}}>
      ...
      {!loading && (<>
        ...
        {tab==='positions' && (
          <div>
            ...
            {positions.length === 0 ? (
              ...
              const forecast = p.forecast_temp;
                  ...
                  <div style={{display:'flex',alignItems:'flex-start',gap:12}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4,flexWrap:'wrap'}}>
                        <span style={{fontSize:13.5,fontWeight:700,color:'var(--text)',textTransform:'capitalize'}}>{p.city||'?'}</span>
                        ...
                        <span style={{fontSize:12}}>{riskLevel}</span>
                      </div>
                      {/* Météo */}
                      {(forecast || gap != null) && (
                        <div style={{display:'flex',gap:12,marginBottom:6,fontSize:11.5,color:'var(--text-3)'}}>
                          {forecast && <span>🌡️ Prévision : <strong style={{color:'var(--text)'}}>{forecast}°F</strong></span>}
                      ...
                      {/* Analyse */}
                      {p.analysis && (
                        <div style={{fontSize:12.5,color:'var(--text-2)',lineHeight:1.6,padding:'8px 12px',borderRadius:8,background:'var(--fill)',borderLeft:'3px solid color-mix(in oklab,var(--accent) 40%,transparent)'}}>
                          {p.analysis}
                        </div>
                      )}
        ...
        {tab==='rapport' && (
          <div>
            {rapports.length === 0 ? (
              <Card style={{padding:28,textAlign:'center',color:'var(--text-3)'}}>
                🧠 Le rapport sera généré après le premier cycle complet<br/>
                ...
                {i===0 && (
                  <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:12}}>
                    {[
                      ...
                      {l:'NO',v:`${r.nb_no} (${(r.prix_no_moyen*100).toFixed(0)}¢)`,c:'#6366f1'},
                      {l:'YES',v:`${r.nb_yes} (${(r.prix_yes_moyen*100).toFixed(0)}¢)`,c:'#f59e0b'},
                    ].map((s,j)=>(
